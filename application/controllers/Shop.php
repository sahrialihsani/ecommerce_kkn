    ...
    public function checkout($action = '')
    {
        ...
        switch ($action)
        {
                    ...
                    if ($this->customer->is_coupon_exist($coupon))
                    {
                        if ($this->customer->is_coupon_active($coupon))
                        {
                            ...
                            else
                            {
                                ...
                                $subtotal = $this->input->post('subtotal'); //mendapatkan subtotoal yang sudah disimpan sementara
                                if($credit>=$subtotal):
                                    $discount = $subtotal;
                                else:
                                    $discount = $credit;
                                endif;
                                $disc = '<span class="badge badge-success">'. $coupon .'</span> Rp '. format_rupiah($discount);
                            }
                        }
                        else
                        {
            ...
            case 'order' : /////INI ORDER
                ...
                foreach ($quantity as $id => $data)
                {
                    $oldStock=$this->product->get_product_by_id($id)->stock;
                    $updatedItems[$n]['id'] = $id;
                    $updatedItems[$n]['stock'] = $oldStock-$data['qty'];
                    if($oldStock-$data['qty']<=0){
                        $updatedItems[$n]['is_available'] = NULL;
                    }else{
                        $updatedItems[$n]['stock'] = $oldStock-$data['qty'];
                    }
                ...
                $this->product->update_ordered_items($updatedItems);
                ////////////////////////
