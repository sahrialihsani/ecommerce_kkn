...
class Payments extends CI_Controller {
    ...
    public function _get_payment_list()
    { 
        $payments = $this->payment->get_all_payments_for_api();
        $n = 0;

        foreach ($payments as $payment)
        {
            $payments[$n]->id = ($payment->id);
            $payments[$n]->order_number = ($payment->order_number);
            $payments[$n]->customer = ($payment->customer);
            $payments[$n]->payment_date = get_formatted_date($payment->payment_date);
            $payments[$n]->payment_price = "Rp ". ' ' . number_format($payment->payment_price, 0 ,',', '.');
            $payments[$n]->status =  ($payment->status == 1) ? 'Menunggu Konfirmasi' : (($payment->status == 2)? 'Dikonfirmasi' : 'Gagal');
            $n++;
        }
    ...
    public function api($action = '')
    {
        ...
        {
            case 'payments' :
                $payments['data'] = $this->_get_payment_list();
